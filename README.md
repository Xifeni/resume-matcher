...
На графиках сравнения отчётливо видно, что нам удалось добиться решить главное. С помощью калибровки повторять тренд экспертной(ручной разметки) выводить вперёд по ранджированию подходящих кандидатов и отсекать однозначно не подходящих. Это подверждается изображением ниже основанном на тестовой выборке, хоть она и не объёмна.

![Сравнение реальных и предсказанных значений(по индексам) только тестовая выборка](images/vall2.png)
...
- Не использовать одну универсальную ML-модель для всех вакансий, а обучать отдельный калибратор для каждой экспертной области (IT, маркетинг, финансы, производство и т.д.). Активация отдельно модели
